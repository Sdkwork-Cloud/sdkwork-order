...
//! ```
//!
//! 成功响应使用 `SdkWorkApiResponse`（`API_SPEC.md` §15.1.1，`code: 0`，`data`，`traceId`）。
//! 错误响应使用 `application/problem+json`（`SdkWorkProblemDetail`，§15.2）。
...
mod tests {
    ...
    fn problem_response_includes_required_fields() {
        ...
        assert_eq!("refund conflict", payload["detail"].as_str().unwrap());
        assert_eq!("4090", payload["code"].as_str().unwrap());
    }
}
