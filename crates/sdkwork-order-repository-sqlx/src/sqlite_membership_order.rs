...
impl SqliteCommerceMembershipOrderStore {
    ...
    async fn load_membership_order_by_idempotency_key(
        ...
        let row = sqlx::query(
            ...
            SELECT
                ...
                COALESCE(
                    NULLIF(json_extract(COALESCE(oi.sku_snapshot_json, '{}'), '$.packageId'), ''),
                    ?
                ) AS package_id,
                COALESCE(
                    NULLIF(json_extract(COALESCE(oi.sku_snapshot_json, '{}'), '$.productName'), ''),
                    oi.title,
                    'Membership package'
                ) AS package_name,
                CAST(COALESCE(
                    NULLIF(json_extract(COALESCE(oi.sku_snapshot_json, '{}'), '$.durationDays'), ''),
                    '0'
