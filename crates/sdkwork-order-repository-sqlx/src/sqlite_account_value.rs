...
use crate::sqlite_recharge::SqliteCommerceRechargeStore;
...
impl SqliteCommerceRechargeStore {
    ...
    pub async fn upsert_account_value_package(
        ...
        let mut tx = self
            ...
            .await
            .map_err(|error| store_error("failed to begin account value package upsert", error))?;
        ...
        let updated = sqlx::query(
            ...
            SET display_name = ?,
                ...
                idempotency_key = ?,
                updated_at = ?,
                retired_at = CASE WHEN ? = 'retired' THEN COALESCE(retired_at, ?) ELSE NULL END
            WHERE tenant_id = CAST(? AS TEXT)
              AND ((organization_id = CAST(? AS TEXT)) OR (organization_id IS NULL AND ? IS NULL))
              AND (id = CAST(? AS TEXT) OR package_code = CAST(? AS TEXT))
            "#,
        )
            ...
            sqlx::query(
                ...
                VALUES
                    (?, CAST(? AS TEXT), CAST(? AS TEXT), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,
                     CASE WHEN ? = 'retired' THEN ? ELSE NULL END)
                "#,
            )
            .bind(&package_id)
            .bind(&command.tenant_id)
    ...
    async fn update_refund_request_status(
        ...
        let updated = sqlx::query(
            ...
            SET status = ?,
                ...
                account_effect_reference_id = COALESCE(?, account_effect_reference_id),
                request_no = ?,
                idempotency_key = ?,
                updated_at = ?
            WHERE tenant_id = CAST(? AS TEXT)
    ...
    async fn load_account_value_order_by_idempotency(
        ...
        let row = sqlx::query(
            ...
            SELECT
                o.id AS order_id,
                o.order_no,
                COALESCE(NULLIF(json_extract(COALESCE(oi.sku_snapshot_json, '{}'), '$.outTradeNo'), ''), o.order_no) AS out_trade_no,
                o.subject,
                COALESCE(NULLIF(json_extract(COALESCE(oi.sku_snapshot_json, '{}'), '$.targetAsset'), ''), '') AS target_asset,
                CAST(COALESCE(b.payable_amount, '0') AS TEXT) AS amount,
                CAST(COALESCE(NULLIF(json_extract(COALESCE(oi.sku_snapshot_json, '{}'), '$.grantAmount'), ''), '0') AS TEXT) AS grant_amount,
                o.currency_code,
                o.status
...
impl AccountValueRequestExecutionStore for SqliteCommerceRechargeStore {
...
async fn insert_account_value_order(
    ...
    sqlx::query(
        ...
        VALUES
            (?, CAST(? AS TEXT), CAST(? AS TEXT), CAST(? AS TEXT), ?, 'pending_payment', 'pending',
             'unfulfilled', 'none', ?, ?, ?, ?, ?, NULL, NULL, ?, ?)
        "#,
    )
    ...
    sqlx::query(
        ...
        INSERT INTO commerce_order
            (id, tenant_id, organization_id, owner_user_id, order_no, status, payment_status,
             ...
             idempotency_key, created_at, paid_at, cancelled_at, expired_at, updated_at)
        VALUES
            (?, CAST(? AS TEXT), CAST(? AS TEXT), CAST(? AS TEXT), ?, ?, ?, 'unfulfilled',
             'none', 'coupon_recharge', ?, ?, ?, ?, NULL, NULL, ?, ?)
        "#,
    )
    .bind(&command.order_id)
    .bind(&command.tenant_id)
...
async fn insert_account_value_amount_breakdown(
    ...
    sqlx::query(
        ...
        VALUES
            (?, CAST(? AS TEXT), CAST(? AS TEXT), ?, NULL, 'order_total', ?, '0', ?, ?, ?)
        "#,
    )
...
fn total_count_cell(row: &sqlx::sqlite::SqliteRow) -> i64 {
