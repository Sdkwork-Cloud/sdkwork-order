...
export async function applySdkworkIdempotencyRequestFingerprint(
...
function withoutHeaders(
  ...
  return Object.fromEntries(
    Object.entries(headers ?? {}).filter(
      ([headerName]) => !normalizedNames.has(headerName.toLowerCase()),
    ),
  );
}

async function createRequestBodyFingerprint(
  body: unknown,
): Promise<{
  header: typeof CONTENT_SHA256_HEADER | typeof IDEMPOTENCY_FINGERPRINT_HEADER;
  value: string;
} | undefined> {
...
async function serializeRequestBodyBytes(body: unknown): Promise<Uint8Array | undefined> {
