...
$InputPath = Join-Path $FamilyRoot "openapi\sdkwork-order-backend-api.openapi.json"
$ApiPrefix = "/backend/v3/api"
$ClientName = "SdkworkOrderBackendClient"
...
foreach ($LanguageValue in $Languages) {
    foreach ($LanguagePart in "$LanguageValue".Split(",")) {
        ...
        $PackageName = "@sdkwork/order-backend-sdk"
