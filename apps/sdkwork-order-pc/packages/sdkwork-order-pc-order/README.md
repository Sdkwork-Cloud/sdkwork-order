...
- `@sdkwork/order-service` for generated app/backend SDK boundaries, session checks, and response normalization
...
All remote commerce access goes through `@sdkwork/order-service` or through sibling commerce services that use the same boundary. Generated SDK clients remain behind the shared service contract.
...
- `pnpm --filter @sdkwork/order-pc-order typecheck`
